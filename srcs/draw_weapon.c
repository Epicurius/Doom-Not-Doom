...
/*   Updated: 2021/06/17 14:28:33 by nneronin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_weapon(t_doom *doom)
{
	...
	while (++y < 10)
	{
		...
		thread[y].srcr = rect_xywh(srcr.x1, srcr.y1
				+ (y * (srcr.h / 10)), srcr.w, srcr.h / 10);
		thread[y].dst = doom->surface;
		thread[y].dstr = rect_xywh(dstr.x1, dstr.y1
				+ (y * (dstr.h / 10)), dstr.w, dstr.h / 10);
