...
void	 project_projectile(t_doom *doom, t_sprite_render *render)
{
...
void	rotate_projectile(t_doom *doom, t_projectile *orb, t_sprite_render *render)
{
	t_xyz dist;
	t_xyz screen;
...
void 	DrawProjectiles(t_doom *doom)
{
	...
	while (curr != NULL)
	{
		...
		if (doom->sectors[orb->sector].visible)
		{
			...
			project_projectile(doom, &render);
			blit_sprite(&render);
		}
		curr = curr->next;
	}
