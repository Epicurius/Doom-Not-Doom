...
/*   Updated: 2021/09/18 14:21:37 by nneronin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	curr_floor_and_ceiling(t_doom *doom, t_wall *w)
{
	...
	w->stat_y1.floor = doom->c.y + (v1.floor + w->angle_z1) * w->scale_c1z;
	w->stat_y1.ceiling = doom->c.y + (v1.ceiling + w->angle_z1) * w->scale_c1z;
	w->stat_y2.floor = doom->c.y + (v2.floor + w->angle_z2) * w->scale_c2z;
	w->stat_y2.ceiling = doom->c.y + (v2.ceiling + w->angle_z2) * w->scale_c2z;
...
static void	slope_curr_floor_and_ceil(t_doom *doom, t_wall *w, t_v3 p1, t_v3 p2)
{
	...
	w->incl_y1.floor = doom->c.y + (v1.floor + w->angle_z1) * w->scale_c1z;
	w->incl_y1.ceiling = doom->c.y + (v1.ceiling + w->angle_z1) * w->scale_c1z;
	w->incl_y2.floor = doom->c.y + (v2.floor + w->angle_z2) * w->scale_c2z;
	w->incl_y2.ceiling = doom->c.y + (v2.ceiling + w->angle_z2) * w->scale_c2z;
...
static void	neighbour_floor_and_ceil(t_doom *doom, t_wall *w, t_v3 p1, t_v3 p2)
{
	...
	w->incl_ny1.floor = doom->c.y + (v1.floor + w->angle_z1) * w->scale_c1z;
	w->incl_ny1.ceiling = doom->c.y + (v1.ceiling + w->angle_z1) * w->scale_c1z;
	w->incl_ny2.floor = doom->c.y + (v2.floor + w->angle_z2) * w->scale_c2z;
	w->incl_ny2.ceiling = doom->c.y + (v2.ceiling + w->angle_z2) * w->scale_c2z;
...
void	project_wall(t_doom *doom, t_wall *wall)
{
	...
	wall->cx1 = wall->cv1.x * wall->scale_c1z + doom->c.x;
	wall->cx2 = wall->cv2.x * wall->scale_c2z + doom->c.x;
